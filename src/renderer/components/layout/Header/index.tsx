  ...
  return (
    <header class={styles['app-header']}>
      ...
      <div class={styles['header-left']}>
        <span class={styles['app-name']}>青植时间</span>
      </div>
      ...
      <nav class={styles['header-nav']}>
        {views.map((view) => {
          ...
          return (
            <button
              key={view.id}
              class={`${styles['nav-item']} ${isActive ? styles.active : ''}`}
              onClick={() => handleViewChange(view.id)}
            >
              <img src={view.icon} alt={view.label} class={styles['nav-icon']} />
      ...
      <div class={styles['header-right']}>
        <button class={styles['header-btn']} onClick={handleMinimize} title="最小化">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        </button>
        <button class={styles['header-btn']} onClick={handleMaximize} title="最大化">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
          </svg>
        </button>
        <button class={`${styles['header-btn']} ${styles['close-btn']}`} onClick={handleClose} title="关闭">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
